    ...
    def forward(self, logits: torch.Tensor, outputs: torch.Tensor, gt_mask: torch.Tensor, gt_skel: torch.Tensor, **batch):
        ...
        for i in range(n_heads):
            ...
            if l_i.shape[2:] != gt_mask.shape[1:]:
                gt_m_i = F.interpolate(gt_mask.unsqueeze(1).float(), size=l_i.shape[2:], mode='nearest').squeeze(1).long()
                gt_s_i = F.interpolate(gt_skel.unsqueeze(1).float(), size=l_i.shape[2:], mode='nearest').squeeze(1)
            else:
