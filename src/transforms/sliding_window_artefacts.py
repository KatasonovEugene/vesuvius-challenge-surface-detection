...
class RandPatchProbReduction(nn.Module):
    ...
    def forward(self, teacher_probs, **batch):
        ...
        patch_sizes = torch.randint(self.patch_size_range[0], self.patch_size_range[1], size=(B,), device=device)
        start_ds = torch.randint(0, D - patch_sizes, size=(B,), device=device)
        start_hs = torch.randint(0, H - patch_sizes, size=(B,), device=device)
        start_ws = torch.randint(0, W - patch_sizes, size=(B,), device=device)
