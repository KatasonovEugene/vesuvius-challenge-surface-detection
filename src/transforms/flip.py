...
class RandFlip3D(nn.Module):
    ...
    def forward(self, volume, gt_mask, gt_skel, **batch):
        ...
        flip = lambda x : torch.flip(x[apply_transform], dims=[self.spatial_axis])
        volume[apply_transform] = flip(volume)
        gt_mask[apply_transform] = flip(gt_mask)
        gt_skel[apply_transform] = flip(gt_skel)
